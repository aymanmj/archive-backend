...
export class IncomingService {
  ...
  // ----------------------------------------
  // Helper: تحويل سجل وارد لصيغة مبسطة للواجهة
  // ----------------------------------------
  private mapIncomingLite(rec: any) {
    return {
      ...
      // ✅ حساب وجود مرفقات بالاعتماد على إرجاع "ملف واحد فقط" (take:1)
      hasFiles:
        Array.isArray(rec.document?.files) &&
        rec.document.files.length > 0,

      // حالة التوزيع الأخيرة (إن وُجدت)
      currentStatus:
        rec.distributions && rec.distributions.length > 0
  ...
  // ----------------------------------------
  // قائمة أحدث الوارد (للقائمة الرئيسية)
  // ----------------------------------------
  async listLatestForUser(limit = 20) {
    const rows = await this.prisma.incomingRecord.findMany({
      ...
      select: {
        ...
        // ✅ نعيد ملفًا واحدًا فقط للتحقق من وجود مرفقات
        document: {
          select: {
            id: true,
            title: true,
  ...
  async createIncoming(input: {
    ...
    // إعدادات أساسية خارج المعاملة
    const sec = await this.prisma.securityLevel.findFirst({
      where: { levelName: 'Internal' }, // عدّل الاسم لو مختلف عندك
      select: { id: true },
    });
    if (!sec) {
    ...
    const createdIncoming = await this.prisma.$transaction(async (tx) => {
      ...
      // 3) توليد رقم وارد (YYYY/000001) حسب السنة
      ...
      const newIncomingNumber = `${year}/${padded}`;

      // 4) IncomingRecord + أول توزيع
      const now = new Date();
      const incomingRecord = await tx.incomingRecord.create({
        ...
        // ما نحتاجه لرد الواجهة بعد الإنشاء
        select: {
          id: true,
          incomingNumber: true,
          receivedDate: true,
  ...
  // ----------------------------------------
  // تفاصيل وارد واحد (لصفحة التفاصيل)
  // ----------------------------------------
      ...
      document: rec.document
        ? {
            ...
            files: rec.document.files.map((f: any) => ({
              ...
              // ✅ رابط العرض عبر ServeStaticModule (/uploads)
  ...
  async listForDepartment(departmentId: number) {
    const rows = await this.prisma.incomingRecord.findMany({
      orderBy: { receivedDate: 'desc' },
      where: {
        distributions: { some: { targetDepartmentId: departmentId } },
      },
      select: {
        id: true,
        incomingNumber: true,
        ...
        document: {
          select: {
            ...
            summary: true,
            owningDepartment: { select: { id: true, name: true } },
            // ✅ يكفي ملف واحد لمعرفة وجود مرفقات
            files: { select: { id: true }, take: 1 },
          },
        },
