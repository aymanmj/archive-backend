...
export class EscalationWorker {
  ...
  async run() {
    ...
    for (const d of overdue) {
      try {
        const elapsedMin = Math.floor(
          (now.getTime() - (d.dueAt as Date).getTime()) / 60000,
        );
        ...
        if (lastEscLog) {
          const minsSince = Math.floor(
            (now.getTime() - lastEscLog.createdAt.getTime()) / 60000,
          );
        ...
        const newPriority = Math.min(
          10,
          (d.priority ?? 0) + (nextLevel.priorityBump ?? 0),
        );
        const newStatus: DistributionStatus =
          nextLevel.statusOnReach || d.status;
        ...
        await this.prisma.$transaction(async (tx) => {
          ...
          await tx.incomingDistributionLog.create({
            data: {
              ...
              note: `ESC:L${nextLevel.level} — over ${elapsedMin} min, due=${(
                d.dueAt as Date
              ).toISOString()}`,
        ...
        // 3) كتابة حدث في الـ Timeline للوارد
        const documentId = d.incoming?.documentId ?? d.incoming?.id ?? null;
        ...
        if (documentId) {
          await this.timeline.record({
            ...
            details: {
              ...
              newStatus,
              newPriority,
              assignedToUserId: assignTo ?? d.assignedToUserId ?? null,
              dueAt: (d.dueAt as Date).toISOString(),
            },
          });
        }

        // 4) Hooks إشعار (حاليًا: لوج فقط)
        // 5) إشعارات التصعيد
        if (
          nextLevel.notifyAssignee ||
          nextLevel.notifyManager ||
          nextLevel.notifyAdmin
        ) {
          const notifyUserIds: number[] = [];

          // المكلّف الفعلي (الجديد لو حصل autoReassign وإلا القديم)
          const effectiveAssignee = assignTo ?? d.assignedToUserId;
          if (nextLevel.notifyAssignee && effectiveAssignee) {
            notifyUserIds.push(effectiveAssignee);
          }

          // مدير القسم
          if (nextLevel.notifyManager && d.targetDepartmentId) {
            const managerId = await this.pickManagerForDepartment(
              d.targetDepartmentId,
            );
            if (managerId) notifyUserIds.push(managerId);
          }

          // الإداريون (ADMIN)
          if (nextLevel.notifyAdmin && adminIds.length) {
            notifyUserIds.push(...adminIds);
          }

          await this.notifyEscalation(notifyUserIds, {
            distributionId: Number(d.id),
            level: nextLevel.level,
            newPriority,
            incomingId: Number(d.incoming.id) ?? null,
            incomingNumber: d.incoming?.incomingNumber ?? null,
          });
        }
        ...
        this.logger.error(
          `Escalation error for dist ${d.id}: ${
    ...
    if (processed) {
      this.logger.log(
        `Escalation run completed: processed ${processed} distributions.`,
      );
    }
  }
  ...
  private async notifyEscalation(
    ...
    const title = `تصعيد توزيع #${data.distributionId}`;
    const docPart = data.incomingNumber
      ? ` (الوارد رقم ${data.incomingNumber})`
      : '';
    const body = `تم تصعيد توزيع رقم ${data.distributionId}${docPart} إلى المستوى L${data.level}، الأولوية الحالية ${data.newPriority}.`;

    const link = data.incomingId ? `/incoming/${data.incomingId}` : undefined;

    // نختار شدة التنبيه حسب المستوى
    const severity: NotificationSeverity =
      data.level >= 4 ? 'danger'
      : data.level >= 2 ? 'warning'
      : 'info';
