...
//   // تأكد من وجود الدورين
//   const incomingViewer = await prisma.role.upsert({
//     where: { roleName: 'ViewerIncoming' },
//     update: { description: 'عرض وارد فقط' },
//     create: { roleName: 'ViewerIncoming', description: 'عرض وارد فقط' },
//   });
//   const outgoingViewer = await prisma.role.upsert({
//     where: { roleName: 'ViewerOutgoing' },
//     update: { description: 'عرض صادر فقط' },
//     create: { roleName: 'ViewerOutgoing', description: 'عرض صادر فقط' },
//   });

//   // احصل على IDs الصلاحيات المطلوبة
//   const need = ['incoming.read', 'outgoing.read', 'files.read'];
//   const perms = await prisma.permission.findMany({ where: { code: { in: need } } });

//   // اربط المطلوبة لكل دور
//   const link = async (roleId: number, codes: string[]) => {
//     const subset = perms.filter(p => codes.includes(p.code));
//     await prisma.rolePermission.deleteMany({ where: { roleId } });
//     await prisma.$transaction(
//       subset.map(p => prisma.rolePermission.create({ data: { roleId, permissionId: p.id } }))
//     );
//   };

//   await link(incomingViewer.id, ['incoming.read', 'files.read']);
//   await link(outgoingViewer.id, ['outgoing.read', 'files.read']);
// }
